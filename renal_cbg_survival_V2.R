...
simpleSurvivalPlot_cv<-function(inputFrame,endDateUnix,ylimMin) {
  # inputFrame <- survival_oneRowPerID[n_cbg_duringWindow > n_cbg]
  # endDateUnix <- max(survival_oneRowPerID[n_cbg_duringWindow > n_cbg]$unix_deathDate)
  # sampleDateUnix <- 
  ...
  # medianCV = 0.36
  medianCV <- quantile(SurvivalData$cv_glucose_during_Window, na.rm = T)[3]
    print(medianCV)
  medianIQR <- quantile(SurvivalData$IQR_glucose_during_Window, na.rm = T)[3]
  
  
  
  mfitAge50<-survfit(Surv(timeToDeathInterval, shortDeathEvent) ~ (cv_glucose_during_Window > medianCV), data = SurvivalData)
  shortPlotTitle <- paste("Mortality, time ",round(shortCensorPeriodStartDay)/DaySeconds," to ",round(max(SurvivalData$timeToDeathInterval))/DaySeconds," days\n n= ",nrow(SurvivalData),", threshold: ",quantile(SurvivalData$hba1cIQRinRange)[3],sep="")
  plot(mfitAge50,mark.time=T,lty=1:6,conf.int=F,col=c("black","red","blue","green","orange","purple"),main=shortPlotTitle,xlim=c(shortCensorPeriodStartDay,round(max(SurvivalData$timeToDeathInterval))),lwd=5,ylim=c(ylimMin,1))
  
  # mfitAge50.coxph<-coxph(Surv(timeToDeathInterval, shortDeathEvent) ~ age_atSampleTime+medianHbA1cInRange+nValsPerIDinRange+(hba1cIQRinRange>=quantile(SurvivalData$hba1cIQRinRange)[3]), data = SurvivalData)
  
  mfitAge50.coxph<-coxph(Surv(timeToDeathInterval, shortDeathEvent) ~ age_starting_RRT + CBGrate + (cv_glucose_during_Window > medianCV), data = SurvivalData)
  pVal <- summary(mfitAge50.coxph)$coef[,5]; HR <- round(exp(coef(mfitAge50.coxph)),2)
  legendText <- paste("p = ",pVal," | HR = ",HR,sep="")
  summarySurvfit <- summary(mfitAge50); legendNames <- row.names(summarySurvfit$table)
  legend("bottomleft",c(legendNames),lty=1:6,col=c("black","red","blue","green","orange","purple"),cex=0.8); legend("topright",legendText,cex=0.6)
...
intervalToDetermineAdmissionDays <- 5
windowForAnalysis_months = 3
  windowSeconds = windowForAnalysis_months * (60*60*24*(365.25 / 12))
...
# select CBGs within x month window of first CBG post initiation of RRT
  # only CBGs post RRT
  renal_cbgMergeDT[, c('flagPostRRT') := ifelse(dateplustime1 > unix_dateStartingRRT, 1, 0), by = .(CHI)]
  renal_cbgMergeDT <- renal_cbgMergeDT[flagPostRRT == 1]
  # within window set above
  renal_cbgMergeDT[, c('withinWindow') := ifelse(dateplustime1 < (min(dateplustime1) + windowSeconds), 1, 0), by = .(CHI)]
  renal_cbgMergeDT <- renal_cbgMergeDT[withinWindow == 1]
  
  
# order by ID, then by dateplustime1 
# generate metrics on all CBG within window
...
renal_cbgMergeDT$cv_glucose_during_admission <- renal_cbgMergeDT$sd_glucose_during_admission / renal_cbgMergeDT$mean_glucose_during_admission
  ...
  renal_cbgMergeDT[, c('n_cbg_duringWindow') := .N , by=.(CHI)]
  renal_cbgMergeDT[, c('cbg_in_sequence_duringWindow') := seq(1, .N, 1) , by=.(CHI)]
  
  renal_cbgMergeDT[, c('min_glucose_during_Window') := min(yyyy) , by=.(CHI)]
  renal_cbgMergeDT[, c('max_glucose_during_Window') := max(yyyy) , by=.(CHI)]
  renal_cbgMergeDT[, c('median_glucose_during_Window') := quantile(yyyy)[3] , by=.(CHI)]
  renal_cbgMergeDT[, c('IQR_glucose_during_Window') := quantile(yyyy)[4] - quantile(yyyy)[2] , by=.(CHI)]
  renal_cbgMergeDT[, c('mean_glucose_during_Window') := mean(yyyy) , by=.(CHI)]
  renal_cbgMergeDT[, c('sd_glucose_during_Window') := sd(yyyy) , by=.(CHI)]
  renal_cbgMergeDT$cv_glucose_during_Window <- renal_cbgMergeDT$sd_glucose_during_Window / renal_cbgMergeDT$mean_glucose_during_Window
...
singleRowPerAdmission[, c('totalAdmissionDuration') := sum(admissionDurationDays), by = .(CHI)]
singleRowPerAdmission[, c('total_nCBG') := sum(n_cbg_duringAdmission), by = .(CHI)]

singleRowPerAdmission$CBGrate <- ifelse(singleRowPerAdmission$totalAdmissionDuration > 0, singleRowPerAdmission$total_nCBG /singleRowPerAdmission$totalAdmissionDuration, 0)


singleRowPerID <- singleRowPerAdmission[cbg_in_sequence_duringWindow == 1]
singleRowPerID <- singleRowPerID[order(singleRowPerID$CHI, singleRowPerID$admissionNumber)]
...
#n = 1
hypoThresh = 3
singleRowPerID_analysisSet <- singleRowPerID
...
testGroup <- singleRowPerID_analysisSet
testGroup$age_starting_RRT <- testGroup$unix_dateStartingRRT - testGroup$dob.as.num

# ? hypo during the window
testGroup[, c('hypoDuringTestRunIn_perWindow') := ifelse(min_glucose_during_Window < hypoThresh, 1, 0), by = .(CHI)]
...
survival_oneRowPerID <- testGroup

dim(survival_oneRowPerID)
sum(survival_oneRowPerID$hypoDuringTestRunIn_perWindow)

survival_oneRowPerID$hypoDuringTestRunIn_perID <- survival_oneRowPerID$hypoDuringTestRunIn_perWindow
...
simpleSurvivalPlot(survival_oneRowPerID[onSCIDM == 0], max(survival_oneRowPerID[onSCIDM == 0]$unix_deathDate), 0)
simpleSurvivalPlot(survival_oneRowPerID[onSCIDM == 1], max(survival_oneRowPerID[onSCIDM == 1]$unix_deathDate), 0)
...
n_cbg = 6
simpleSurvivalPlot_cv(survival_oneRowPerID[n_cbg_duringWindow > n_cbg], max(survival_oneRowPerID[n_cbg_duringWindow > n_cbg]$unix_deathDate), 0)

simpleSurvivalPlot_cv(survival_oneRowPerID[n_cbg_duringWindow > n_cbg & onSCIDM == 0], max(survival_oneRowPerID[n_cbg_duringWindow > n_cbg & onSCIDM == 0]$unix_deathDate), 0)
simpleSurvivalPlot_cv(survival_oneRowPerID[n_cbg_duringWindow > n_cbg & onSCIDM == 1], max(survival_oneRowPerID[n_cbg_duringWindow > n_cbg & onSCIDM == 1]$unix_deathDate), 0)

#simpleSurvivalPlot_cv(survival_oneRowPerID[n_cbg_duringWindow > 1], max(survival_oneRowPerID[n_cbg_duringWindow > 1]$unix_deathDate), 0)
